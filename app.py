...
def find_best_format(info_dict, height):
    ...
    for f in info_dict.get('formats', []):
        # Filtrar por altura e ter stream de vídeo
        if f.get('height') == height and f.get('vcodec') != 'none':
            ...
            # Preferir formatos com áudio e vídeo combinados
            if f.get('acodec') != 'none':
                if current_tbr > best_tbr:
                    best_format = f
                    best_tbr = current_tbr
            # Se não há um formato combinado melhor, considerar formatos apenas de vídeo (DASH)
            ...
            if not selected_video_data:
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                    ...
                    for idx, future in enumerate(concurrent.futures.as_completed(future_to_video_item)):
                        ...
                        if video_info and (format_720p or format_1080p):
                            st.session_state.download_statuses[url] = 'pending'
                            # Define o valor inicial para o radio de resolução
                            if format_720p:
                                st.session_state[f"res_choice_{url}"] = "720p"
                            elif format_1080p:
                                st.session_state[f"res_choice_{url}"] = "1080p"
                        else:
...
if st.session_state.processed_videos_data:
    ...
    if st.session_state.batch_download_in_progress:
        ...
        if st.session_state.batch_current_index < st.session_state.batch_total_videos:
            ...
            if current_status not in ('completed', 'error', 'error_info_fetch', 'error_no_formats'):
                ...
                # Pega a resolução escolhida que foi salva no st.session_state (do radio)
                chosen_resolution_str = st.session_state.get(f"res_choice_{video_url_to_download}", None)
                
                if not chosen_resolution_str:
    ...
    for i, video_url in enumerate(sorted_processed_urls):
        ...
        format_720p = video_data['format_720p']
        format_1080p = video_data['format_1080p']
        ...
        if video_info:
            resolution_options = {}
            if format_720p:
                resolution_options["720p"] = f"720p - {get_format_size(format_720p)}"
            if format_1080p:
                resolution_options["1080p"] = f"1080p - {get_format_size(format_1080p)}"
            
            if not resolution_options:
                st.warning("Nenhuma resolução 720p ou 1080p disponível detectada para esta URL.")
            ...
            chosen_resolution_str = st.radio(
                ...
                format_func=lambda k: resolution_options[k], # Mostra o display_string (ex: "720p - 50MB")
                index=default_res_idx,
                key=f"res_choice_{video_url}" # O valor do radio é automaticamente armazenado aqui (ex: "720p")
            )
            ...
            if download_status == 'pending':
                ...
                if st.button(f"Baixar '{chosen_resolution_str}' de '{clean_page_title}'", key=f"download_btn_{video_url}", disabled=download_button_disabled):
                    ...
                    st.session_state.processed_videos_data[video_url]['chosen_resolution_str'] = chosen_resolution_str
                ...
                try:
                    ...
                    dl_chosen_resolution_int = st.session_state.processed_videos_data[video_url]['chosen_resolution_int']
                    
                    ydl_opts = {
                        'format': f'bestvideo[height<={dl_chosen_resolution_int}]+bestaudio/best[height<={dl_chosen_resolution_int}]',
            ...
            st.warning(f"Não foi possível obter informações de vídeo ou nenhum formato disponível (720p/1080p) para: {video_url}. Ignorando este vídeo.")
            if download_status in ('error_info_fetch', 'error_no_formats'):
                 ...
                 if st.button(f"Tentar Novamente Obter Info '{clean_page_title}'", key=f"retry_info_btn_{video_url}", disabled=retry_info_button_disabled):
                    ...
                    video_data['format_720p'] = find_best_format(video_data['video_info'], 720)
                    video_data['format_1080p'] = find_best_format(video_data['video_info'], 1080)
                    if video_data['video_info'] and (video_data['format_720p'] or video_data['format_1080p']):
                         st.session_state.download_statuses[video_url] = 'pending'
                         if video_data['format_720p']:
                            st.session_state[f"res_choice_{video_url}"] = "720p"
                         elif video_data['format_1080p']:
                            st.session_state[f"res_choice_{video_url}"] = "1080p"
                    else:
...
# errroooo import streamlit as st
# import requests
# from bs4 import BeautifulSoup
# import yt_dlp
# import os
...
#     st.session_state.base_number = 50
# if 'search_text_for_links' not in st.session_state:
#     st.session_state.search_text_for_links = "view_video" # Novo: texto a ser procurado nos links
...
#             if not f.get('tbr') and not f.get('filesize') and not f.get('filesize_approx'): continue # Pula áudio-only sem informações de tamanho/bitrate
#         elif f.get('vcodec') == 'none' and f.get('acodec') != 'none' and not f.get('height') and not f.get('tbr') and not f.get('filesize') and not f.get('filesize_approx'):
...
#                     # Regex para encontrar "primeira_manter" + "remover" + "ultima_manter"
#                     # E substituir por "primeira_manter" + "ultima_manter"
#                     # O grupo \2 na substituição pega a segunda parte capturada, que é last_k
#                     pattern = rf"({first_k}){text_r}({last_k})"
...
#                                 # Isso define o valor inicial para o selectbox
...
#                 st.markdown("---")
#                 # Se não há formatos, o status deveria ser 'error_no_formats'
#                 if download_status not in ('error_no_formats', 'error_info_fetch'):
#                     st.session_state.download_statuses[video_url] = 'error_no_formats'
#                 continue
